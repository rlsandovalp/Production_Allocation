...
################    PREPROCESSING    ########################
end_members = pd.read_csv('../Data_Base/'+group+'/p_'+dataset+'.csv').set_index('Mix').values.shape[1]
peaks = pd.read_csv('../Data_Base/'+group+'/'+dataset+'.csv')
...
real_results = (pd.read_csv('../Data_Base/'+group+'/p_'+dataset+'.csv')).set_index('Mix').values[end_members:,:]
...
#####################   POSTPROCESSING   #########################
# summm = np.sum(X_todos, axis = 0)
# for cols in range(len(mixtures)):
#         for rows in range(end_members):
#             X_todos[rows,cols] = X_todos[rows,cols]/summm[cols]


######################   CONFIDENCE INTERVALS   #########################
for count, mixture in enumerate(mixtures):
    b = p_m[:,count]
    errors = A @ X_todos[:,count] - b
    for em in range(end_members):
        conf_int_sup[em,count] = X_todos[em,count] + 5
        conf_int_inf[em,count] = X_todos[em,count] - 5
        # conf_int_sup[em,count] = X_todos[em,count] + p_f*(AtA_inv[em,em] * np.var(errors))**0.5
        # conf_int_inf[em,count] = X_todos[em,count] - p_f*(AtA_inv[em,em] * np.var(errors))**0.5
...
plot_results(X_todos,conf_int_inf,conf_int_sup,real_results[mixtures1,:])
