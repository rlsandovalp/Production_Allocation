...
for m, mixture in enumerate(mixtures):
    ...
    tax.scatter(deconvolutions, marker='.', label="Individual Estimates, " r'$x_{k,PGM}$', color = 'gray')
    tax.scatter([true.loc[mixture].values], marker='o', label="True value, " r'$x_{k}^{*}$', color = 'green')
    tax.scatter([averaged.loc[mixture].values], marker='o', label="Estimates by employing all repetitions, " r'$\bar{\bar{x}}_{k,PGM}$', color = 'red')
    tax.scatter([compositional_mean.loc[mixture].values], marker='x', label="Compositional mean, " r'$\bar{x}_{k,PGM}$', color = 'black')
