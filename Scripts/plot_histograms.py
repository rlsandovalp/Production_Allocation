...
for m, mix in enumerate(mixtures):
    x = math.ceil((m+1)/4)-1
    y = (m % 4)
    data = np.loadtxt(folder+'aitchison_distances_'+str(m)+'.txt')
    axes[x,y].hist(data)
    ...
    if (m == 8) or (m == 9) or (m == 7) or (m == 6):
        axes[x,y].set_xlabel(r'$MAPE$')
    mu = np.nanmean(data)
    sigma2 = np.nanvar(data)
    axes[x,y].text(0.7, 0.9, r'$\mu=$'+str(round(mu,1)), transform=axes[x,y].transAxes, fontsize = size_label)
    axes[x,y].text(0.7, 0.8, r'$\sigma^2=$'+str(round(sigma2,1)), transform=axes[x,y].transAxes, fontsize = size_label)
